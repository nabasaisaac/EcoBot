...
def startTruckPi():
    ...
    # Provide shared sonar instance to the active modes and RPC layer
    ...
    threading.Thread(target=MjpgServer.startMjpgServer,
                     daemon=True).start()  # MJPEG streaming server
    ...
    while True:
        ...
        time.sleep(0.03)
        # Execute RPC commands that must run on the main thread
        while True:
            try:
                ...
                break
        #####
        # Run the currently selected mode:
        try:
